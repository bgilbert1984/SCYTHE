...
readonly BUILD_PATH="${UNITY_BUILD_PATH:-$PROJECT_PATH/Builds/Linux}"
readonly BUILD_NAME="${UNITY_BUILD_NAME:-SCYTHE_RF_Sim.x86_64}"
...
find_unity() {
  ...
  for candidate in \
    ...
    "${XDG_DATA_HOME:-$REPO_ROOT/.local/share}"/Unity/Hub/Editor/*/Editor/Unity \
    "${XDG_DATA_HOME:-$REPO_ROOT/.local/share}"/Unity/Hub/Editor/*/Unity; do
    if [[ -x "$candidate" ]]; then
      printf '%s\n' "$candidate"
      return
    fi
...
"$unity_binary" \
  ...
  -buildTarget Linux64 \
  -executeMethod Scythe.Editor.BuildCommand.BuildLinux \
  -buildPath "$BUILD_PATH/$BUILD_NAME" \
  -logFile "$LOG_FILE"
build_exit_code=$?
set -e

if [[ $build_exit_code -ne 0 || ! -x "$BUILD_PATH/$BUILD_NAME" ]]; then
