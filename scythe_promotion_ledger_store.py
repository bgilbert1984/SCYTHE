...
attestation (§9 Amendment A), framed-record parsing (§9), identity
reconstruction (§7, §10), torn-tail detection (§13), generation totals (§11) and
SHADOW seeding (§12).
...
def read_ledger(path: str) -> LedgerRead:
    ...
    for index, line in enumerate(complete):
        ...
        if kind == RESERVED:
            ...
            if seq in reserved:
                return LedgerRead(path=path, readability=LEDGER_UNREADABLE,
                                  records_read=index,
                                  detail=f"record {index}: seq {seq} reserved twice")
    ...
    return LedgerRead(
        ...
        generation=generation, committed=committed, write_failed=failed,
        ...
        detail=(f"tail of {len(tail)} bytes was being written; its identity "
                f"cannot be read" if torn else None))
...
class LedgerStore:
    ...
    def seed_for_shadow(self) -> Tuple[str, ...]:
    ...
    def armed_capability(self) -> Tuple[str, Tuple[str, ...]]:
    ...
    def assessment(self, *, mode: str = "SHADOW") -> Dict[str, Any]:
        ...
        """
        capability, refusals = self.armed_capability()
        read = self.read()
        return {
            "schema": SCHEMA,
            "mode": mode,
            ...
            "shadow_fidelity": self.shadow_fidelity(),
