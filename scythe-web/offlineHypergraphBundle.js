...
ctx.setLineDash([]);ctx.globalAlpha=1;screen=G.nodes.map(n=>({n,p:projected.get(n.id)})).sort((a,b)=>a.p.z-b.p.z);for(const o of screen){const radius=nRadius(o.n)*Math.max(.7,o.p.k)*(o.n.id===selectedId?1.5:1);ctx.beginPath();ctx.arc(o.p.x,o.p.y,radius,0,Math.PI*2);ctx.fillStyle=colors[o.n.evidenceClass]||'#8aa';ctx.fill();ctx.strokeStyle=o.n.id===selectedId?'#fff':'#071422';ctx.lineWidth=o.n.id===selectedId?3:2;ctx.stroke()}positionLabels(projected,w,h)}
...
function summary(n){const e=n.enrichment||{},net=e.network||{},geo=e.geo||{},labels=n.labels||{},lat=Number(geo.latitude),lon=Number(geo.longitude),unc=Number(geo.uncertaintyRadiusKm);return[n.kind||'ENTITY',n.id,'EVIDENCE // '+(n.evidenceClass||'INFERRED'),labels.ip?'IP // '+labels.ip:'',net.asn?'NETWORK // AS'+net.asn+' · '+(net.organization||'UNKNOWN ORG'):'',geo.city?'PLACE ESTIMATE // '+[geo.city,geo.region,geo.country||geo.countryCode].filter(Boolean).join(', '):'',Number.isFinite(lat)&&Number.isFinite(lon)?'COORDINATES // '+lat.toFixed(3)+'°, '+lon.toFixed(3)+'°'+(Number.isFinite(unc)?' · ±'+unc+' km':''):'',n.observedAt?'OBSERVED // '+new Date(n.observedAt*1000).toISOString():'','BOUNDARY // GEOIP IS INFERRED; TOPOLOGY IS NOT GEOLOCATION; ESTIMATE IS NOT PHYSICAL DEVICE LOCATION'].filter(Boolean).join('\\n')}
