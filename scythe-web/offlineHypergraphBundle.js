...
function nn(v){v=Number(v);return Number.isFinite(v)?Math.max(0,v):0}function clamp(v,a,b){return Math.min(b,Math.max(a,v))}
function flowScale(e){const l=e.labels||{},delta=nn(l.motion_forward_delta_packets)+nn(l.motion_reverse_delta_packets),deltaBytes=nn(l.motion_forward_delta_bytes)+nn(l.motion_reverse_delta_bytes),measured=l.motion_basis==='OBSERVED_SURICATA_COUNTER_DELTA'&&(delta>0||deltaBytes>0),packets=measured?delta:nn(l.packets)||(nn(l.flow_pkts_toserver)+nn(l.flow_pkts_toclient)),bytes=measured?deltaBytes:nn(l.bytes)||(nn(l.flow_bytes_toserver)+nn(l.flow_bytes_toclient)),intensity=clamp(Math.log2(1+Math.max(packets,bytes/1500))/12,0,1);return{width:clamp(1.4+intensity*2.6,1.4,6),arrow:clamp(6+intensity*8,6,14)}}
function directionColor(e){const d=String((e.labels||{}).operational_direction||'UNRESOLVED').toUpperCase();return{OUTBOUND:'#00d4ff',INBOUND:'#ff6fb7',EAST_WEST:'#f7d154',EXTERNAL_TRANSIT:'#7890a8',UNRESOLVED:'#fff'}[d]||'#fff'}
...
for(const e of G.edges){const members=(e.nodes||[]).filter(id=>projected.has(id));if(members.length<2)continue;const s=flowScale(e);ctx.strokeStyle=colors[e.evidenceClass]||'#456';ctx.globalAlpha=e.evidenceClass==='INFERRED'?.42:.68;ctx.lineWidth=s.width;ctx.setLineDash(e.evidenceClass==='INFERRED'?[5,5]:[]);const a=projected.get(members[0]);for(const id of members.slice(1)){const b=projected.get(id);ctx.beginPath();ctx.moveTo(a.x,a.y);ctx.lineTo(b.x,b.y);ctx.stroke();const dx=b.x-a.x,dy=b.y-a.y,len=Math.hypot(dx,dy)||1,ux=dx/len,uy=dy/len,mx=a.x+dx*.58,my=a.y+dy*.58,al=s.arrow,aw=al*.48;ctx.beginPath();ctx.moveTo(mx+ux*al*.56,my+uy*al*.56);ctx.lineTo(mx-ux*al*.44-uy*aw,my-uy*al*.44+ux*aw);ctx.lineTo(mx-ux*al*.44+uy*aw,my-uy*al*.44-ux*aw);ctx.closePath();ctx.fillStyle=directionColor(e);ctx.globalAlpha=.96;ctx.fill()}}
...
function nRadius(n){const base=n.kind==='network_host'?4.6:3.5,activity=nn(n.display&&n.display.activityScore),factor=1+Math.min(.55,Math.log2(1+activity)*.22);return Math.min(n.kind==='network_host'?7.2:5.6,base*factor)}function nearest(ev){const r=canvas.getBoundingClientRect(),x=ev.clientX-r.left,y=ev.clientY-r.top;let best=null,dist=14;for(const o of screen){const d=Math.hypot(o.p.x-x,o.p.y-y);if(d<dist){best=o;dist=d}}return best}
