...
def _recorded_module(path):
    """The controls module named in the run's own baseline row, if it says."""
    try:
        with open(path, encoding="utf-8") as handle:
            for line in handle:
                row = json.loads(line)
                if row.get("control") == "BASELINE":
                    return row.get("controls_module")
    except (OSError, ValueError):                      # pragma: no cover
        return None
    return None


def main(path, module=None):
    sys.path.insert(0, os.environ.get("SCYTHE_HARNESS", os.path.dirname(
        os.path.abspath(__file__))))
    # The run says which controls produced it. Defaulting to one slice's module
    # instead made a review of ANOTHER slice report a wrong completeness ratio
    # and a declared broad control as an undeclared detonation --- two lines that
    # read exactly like real failures, from nothing but a forgotten argument.
    # A fact the run can record is not a flag anyone should have to remember.
    if module is None:
        module = _recorded_module(path) or "controls527"
